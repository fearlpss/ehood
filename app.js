...
ui.messages=ui.messages||{};ui.groups=ui.groups||[];ui.nitro=ui.nitro||{};ui.banners=ui.banners||{};ui.groupMeta=ui.groupMeta||{};let chatMessages={};let chatTimer=null;let chatBusy=false;
...
function readFile(f){return new Promise((r,j)=>{const x=new FileReader();x.onload=()=>r(x.result);x.onerror=()=>j(x.error||new Error('Could not read image'));x.readAsDataURL(f)})}
function readImageData(file){return new Promise((resolve,reject)=>{if(!file)return resolve(null);if(!/^image\//i.test(file.type))return reject(new Error('Invalid image'));const img=new Image();const fr=new FileReader();fr.onerror=()=>reject(fr.error||new Error('Could not read image'));fr.onload=()=>{img.onload=()=>{const max=512,scale=Math.min(1,max/Math.max(img.width,img.height)),w=Math.max(1,Math.round(img.width*scale)),h=Math.max(1,Math.round(img.height*scale));const c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(img,0,0,w,h);resolve(c.toDataURL('image/jpeg',0.82))};img.onerror=()=>reject(new Error('Could not decode image'));img.src=fr.result};fr.readAsDataURL(file)})}
...
async function loadGroups(){
 if(localMode){return;}
 const {data,error}=await supa.from('ehood_groups').select('*').order('created_at');
 if(!error) ui.groups=(data||[]).map(g=>({...g,owner_id:g.owner_id}));
}
function vanityForGroup(g){return g?.vanity||g?.id||''}
function normalizeVanity(v){return String(v||'').trim().replace(/^\/+|\/+$/g,'').toLowerCase()}
async function vanityTaken(v,excludeId=''){const vanity=normalizeVanity(v);if(!vanity)return false;if(localMode)return ui.groups.some(x=>String(x.id)!==String(excludeId)&&normalizeVanity(x.vanity)===vanity);if(!supa)throw new Error('Database is not connected.');const {data,error}=await supa.from('ehood_groups').select('id,vanity').ilike('vanity',vanity).limit(50);if(error){console.warn('Vanity check failed:',error);throw error}return (data||[]).some(x=>String(x.id)!==String(excludeId))}
async function updateVanityStatus(inputId,statusId,excludeId=''){const input=$('#'+inputId),el=$('#'+statusId);if(!input||!el)return;const vanity=normalizeVanity(input.value);if(!/^[a-z0-9-]{2,24}$/.test(vanity)){el.textContent=vanity?'Use 2–24 lowercase letters, numbers or hyphens.':'';el.className='muted';return}el.textContent='Checking…';el.className='muted';try{const taken=await vanityTaken(vanity,excludeId);if(normalizeVanity(input.value)!==vanity)return;el.textContent=taken?'Already claimed ✕':'Vanity available ✓';el.className=taken?'error':'available';}catch(e){if(normalizeVanity(input.value)!==vanity)return;el.textContent='Could not verify vanity';el.className='error'}}
function applyVanityRoute(){const path=location.pathname.replace(/^\/+|\/+$/g,'').toLowerCase();if(!path)return;const g=ui.groups.find(x=>String(x.vanity||'').toLowerCase()===path);if(g)activeGroupId=g.id}
function goToGroup(g){if(!g)return;const vanity=vanityForGroup(g);if(vanity&&vanity!==g.id)history.pushState({group:g.id},'',`/${encodeURIComponent(vanity)}`)}
async function openApp(){$('#authView').classList.add('hidden');$('#mainView').classList.remove('hidden');await loadGroups();applyVanityRoute();render()}
...
function renderServers(){const el=$('#servers');if(!el)return;el.innerHTML=ui.groups.map(g=>`<button class="server group-server ${g.id===activeGroupId?'active':''}" title="${esc(g.name)} · /${esc(g.vanity||'')}" onclick="selectGroup('${g.id}')">${g.icon_url?`<img src="${esc(g.icon_url)}" alt="${esc(g.name)}">`:esc(g.icon||g.name[0]||'G')}</button>`).join('')}
function selectGroup(id){activeGroupId=id;dmUser=null;current='general';const g=groupById(id);if(g)goToGroup(g);renderServers();renderChannels();select('general')}
...
function renderChannels(){const names=['general','chat'];const g=currentGroup();const manage=g.id!=='ehood'&&String(g.owner_id)===String(me.id)?`<button class="channel group-delete" onclick="groupSettings()">⚙️ Server Settings</button>`:'';const del=g.id!=='ehood'&&String(g.owner_id)===String(me.id)?`<button class="channel group-delete" onclick="deleteGroup('${g.id}')">🗑️ Delete Group</button>`:'';$('#channels').innerHTML=names.map(n=>`<button class="channel ${n===current&&!dmUser?'active':''}" data-ch="${n}"># ${n}</button>`).join('')+`<button class="channel voice-channel" onclick="joinVoice('${g.id}','General')">🔊 General <span>VOICE</span></button>${manage}${del}`;$$('[data-ch]').forEach(b=>b.onclick=()=>{dmUser=null;select(b.dataset.ch)})}
...
function select(n){current=n;const g=currentGroup();$('#chatTitle').textContent=n;$('#topic').textContent=dmUser?'Direct message':`${g.name} · ${g.vanity&&g.id!=='ehood'?'/'+g.vanity+' · ':''}${n==='general'?'Welcome to Ehood.':'Talk about whatever you want.'}`;const header=document.querySelector('.chat>header');if(header){header.style.backgroundImage=g.banner_url?`linear-gradient(90deg,rgba(10,7,14,.96),rgba(10,7,14,.76)),url('${g.banner_url}')`:'';header.style.backgroundSize='cover';header.style.backgroundPosition='center';}$('#messageInput').placeholder=dmUser?'Message @'+dmUser.username:'Message #'+n;renderChannels();loadChat()}
...
function canCreateServer(){return !!me&&(isOwner()||nitroActive(me))}
function addCommunity(){if(!canCreateServer())return modal(`<div class="create-group"><h2>✨ Ehood Nitro Required</h2><p class="muted">Creating servers is available only to Ehood Nitro members and the Ehood owner.</p><div class="actions"><button class="secondary" onclick="closeModal()">Close</button><button class="primary" onclick="nitroInfo()">Get Ehood Nitro</button></div></div>`);modal(`<div class="create-group"><h2>Create a Server</h2><p class="muted">Build your own Discord-style Ehood server with a custom icon, banner and vanity URL.</p><div class="group-icon-preview" id="groupIconPreview">G</div><input id="groupIconFile" class="hidden-file" type="file" accept="image/*" onchange="previewGroupIcon(event)"><input id="groupBannerFile" class="hidden-file" type="file" accept="image/*"><div class="field"><label>SERVER NAME</label><input id="groupName" maxlength="28" placeholder="My server" oninput="updateGroupIcon()"></div><div class="field"><label>VANITY URL</label><div class="vanity-input"><span>ehood.gg/</span><input id="groupVanity" maxlength="24" placeholder="myserver" oninput="updateVanityStatus('groupVanity','groupVanityStatus')"><small id="groupVanityStatus" class="muted"></small></div><small class="muted">2–24 lowercase letters, numbers or hyphens.</small></div><div class="field"><label>SERVER ICON</label><button class="secondary wide" onclick="document.getElementById('groupIconFile').click()">🖼️ Upload Icon</button></div><div class="field"><label>SERVER BANNER</label><button class="secondary wide" onclick="document.getElementById('groupBannerFile').click()">🌌 Upload Banner</button></div><div class="actions"><button class="secondary" onclick="closeModal()">Cancel</button><button class="primary" onclick="createGroup()">Create Server</button></div></div>`)}
function updateGroupIcon(){const v=($('#groupIcon').value?.trim()||$('#groupName').value.trim()[0]||'G').slice(0,2).toUpperCase();const p=$('#groupIconPreview');if(p&&!p.dataset.custom)p.textContent=v} window.updateGroupIcon=updateGroupIcon;
function previewGroupIcon(e){const f=e.target.files[0];if(f)readImageData(f).then(x=>{const p=$('#groupIconPreview');p.style.backgroundImage=`url('${x}')`;p.style.backgroundSize='cover';p.style.backgroundPosition='center';p.dataset.custom='1';p.textContent=''}) .catch(()=>toast('Could not load that image.'))} window.previewGroupIcon=previewGroupIcon;
async function createGroup(){if(!canCreateServer())return toast('Ehood Nitro or Ehood Owner access is required to create a server.');const name=$('#groupName').value.trim();const vanity=normalizeVanity($('#groupVanity').value);if(!name)return toast('Enter a server name.');if(!/^[a-z0-9-]{2,24}$/.test(vanity))return toast('Vanity must be 2–24 lowercase letters, numbers or hyphens.');let icon_url=null,banner_url=null;const iconFile=$('#groupIconFile')?.files?.[0],bannerFile=$('#groupBannerFile')?.files?.[0];if(iconFile)icon_url=await readImageData(iconFile);if(bannerFile)banner_url=await readImageData(bannerFile);const g={id:crypto.randomUUID(),name,vanity,icon_url,banner_url,icon:name[0].toUpperCase(),owner_id:me.id};if(localMode){ui.groups.push(g);saveUI();}else{const {data,error}=await supa.from('ehood_groups').insert({name,vanity,icon_url,banner_url,owner_id:me.id}).select('*').single();if(error)return toast(error.code==='23505'?'That vanity URL is already claimed.':`Could not create server: ${error.message||'unknown error'}`);Object.assign(g,data);ui.groups.push(g)}activeGroupId=g.id;current='general';closeModal();goToGroup(g);render();toast(`${name} created! ehood.gg/${vanity}`)}
function groupSettings(){const g=currentGroup();if(g.id==='ehood'||String(g.owner_id)!==String(me.id))return toast('Only the server owner can edit this server.');modal(`<div class="create-group"><h2>Server Settings</h2><p class="muted">Customize ${esc(g.name)}.</p><div class="group-icon-preview" id="editGroupIconPreview" style="${g.icon_url?`background-image:url('${esc(g.icon_url)}');background-size:cover;background-position:center`:''}">${g.icon_url?'':esc(g.icon||g.name[0]||'G')}</div><div class="group-banner-editor ${g.banner_url?'has-custom':''}" id="serverBannerPreview" style="${g.banner_url?`background-image:url('${esc(g.banner_url)}')`:''}"></div><input id="editGroupIcon" class="hidden-file" type="file" accept="image/*"><input id="editGroupBanner" class="hidden-file" type="file" accept="image/*"><div class="field"><label>SERVER NAME</label><input id="editGroupName" maxlength="28" value="${esc(g.name)}"></div><div class="field"><label>VANITY URL</label><div class="vanity-input"><span>ehood.gg/</span><input id="editGroupVanity" maxlength="24" value="${esc(g.vanity||'')}" oninput="updateVanityStatus('editGroupVanity','editGroupVanityStatus','${g.id}')"><small id="editGroupVanityStatus" class="muted"></small></div></div><div class="actions"><button class="secondary" onclick="document.getElementById('editGroupIcon').click()">🖼️ Change Icon</button><button class="secondary" onclick="document.getElementById('editGroupBanner').click()">🌌 Change Banner</button></div><div class="actions"><button class="secondary" onclick="closeModal()">Cancel</button><button class="primary" onclick="saveGroupSettings()">Save Server</button></div></div>`);$('#editGroupIcon').onchange=e=>{const f=e.target.files[0];if(f)readImageData(f).then(x=>{const p=$('#editGroupIconPreview');p.style.backgroundImage=`url('${x}')`;p.style.backgroundSize='cover';p.style.backgroundPosition='center';p.textContent=''})};$('#editGroupBanner').onchange=e=>{const f=e.target.files[0];if(f)readImageData(f).then(x=>{$('#serverBannerPreview').style.backgroundImage=`url('${x}')`;$('#serverBannerPreview').classList.add('has-custom')})}}
async function saveGroupSettings(){const g=currentGroup();const name=$('#editGroupName').value.trim(),vanity=normalizeVanity($('#editGroupVanity').value);if(!name||!/^[a-z0-9-]{2,24}$/.test(vanity))return toast('Invalid server name or vanity.');if(await vanityTaken(vanity,g.id))return toast('That vanity URL is already claimed.');let icon_url=g.icon_url,banner_url=g.banner_url;const fi=$('#editGroupIcon').files[0],fb=$('#editGroupBanner').files[0];if(fi)icon_url=await readFile(fi);if(fb)banner_url=await readFile(fb);if(localMode){Object.assign(g,{name,vanity,icon_url,banner_url});saveUI();}else{const {data,error}=await supa.from('ehood_groups').update({name,vanity,icon_url,banner_url}).eq('id',g.id).eq('owner_id',me.id).select('*').single();if(error)return toast(error.code==='23505'?'That vanity URL is already claimed.':`Could not save server: ${error.message||'unknown error'}`);Object.assign(g,data)}closeModal();goToGroup(g);render();toast('Server updated.') }
window.groupSettings=groupSettings;window.saveGroupSettings=saveGroupSettings;window.updateVanityStatus=updateVanityStatus;
...
window.addEventListener('popstate',()=>{applyVanityRoute();if(me){render()}});
